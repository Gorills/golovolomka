...
$(document).on('click','.remove',function(e){
  if ($(this).is('.orders-order-delete, .games-delete-ajax')) {
    return;
  }
  var href = $(this).attr('href')
...
// Заявки / игры: удаление без перезагрузки и пакетное удаление
...
function adminPostJson(url, payload) {
  return $.ajax({
    url: url,
    type: 'POST',
    ...
    beforeSend: function(xhr) {
      xhr.setRequestHeader('X-CSRFToken', adminGetCsrfToken() || '');
    }
  });
}

function ordersAdjustCounts($section, n, isReserve) {
  if (!$section || !$section.length) return;
  var reg = parseInt($section.find('.orders-game-registered').text(), 10);
  var res = parseInt($section.find('.orders-game-reserve').text(), 10);
  if (isNaN(reg)) reg = 0;
  if (isNaN(res)) res = 0;
  reg -= n;
  if (isReserve) res -= n;
  if (reg < 0) reg = 0;
  if (res < 0) res = 0;
  $section.find('.orders-game-registered').text(reg);
  $section.find('.orders-game-reserve').text(res);
}

function ordersSetRequestCount($section, n) {
  if (!$section || !$section.length) return;
  var v = Math.max(0, parseInt(n, 10) || 0);
  $section.find('.orders-game-requests-count').text(v);
  $section.find('.orders-game-toggle__count').text('(' + v + ')');
}

function ordersDecrementRequestCount($section) {
  if (!$section || !$section.length) return;
  var cur = parseInt($section.find('.orders-game-requests-count').first().text(), 10) || 0;
  ordersSetRequestCount($section, cur - 1);
}

$(document).on('click', '.orders-game-toggle', function(e) {
  e.preventDefault();
  var $btn = $(this);
  var $sec = $btn.closest('.orders-game-section');
  var $panel = $sec.find('.orders-game-panel');
  var open = !$sec.hasClass('orders-game-section--open');
  $sec.toggleClass('orders-game-section--open', open);
  $btn.attr('aria-expanded', open ? 'true' : 'false');
  $panel.prop('hidden', !open);
});

function adminApplyHidePast($root) {
  if (!$root || !$root.length) return;
  var on = $root.find('.js-admin-hide-past').first().prop('checked');
  $root.toggleClass('admin-hide-past-on', !!on);
}

$(document).on('change', '.js-admin-hide-past', function() {
  var $root = $(this).closest('.js-admin-past-filter-root');
  adminApplyHidePast($root);
});

$(document).on('change', '.orders-select-all-orders', function() {
  var gid = $(this).data('game-id');
  var checked = $(this).prop('checked');
  $('.orders-select-order[data-game-id="' + gid + '"]').prop('checked', checked);
});

$(document).on('click', '.orders-bulk-orders-btn', function() {
  var $admin = $('.orders-admin');
  var url = $admin.data('orders-bulk-url');
  if (!url) return;
  var ids = $('.orders-select-order:checked').map(function() { return parseInt($(this).val(), 10); }).get().filter(function(x) { return !isNaN(x); });
  if (!ids.length) {
    alert('Отметьте одну или несколько заявок.');
    return;
  }
  if (!confirm('Данное действие необратимо. Удалить выбранные заявки?')) return;
  adminPostJson(url, { ids: ids }).done(function(resp) {
    if (!resp || !resp.ok) return;
    (resp.deleted || []).forEach(function(d) {
      $('.orders-order-row[data-order-id="' + d.id + '"]').remove();
      var $sec = $('.orders-game-section[data-game-id="' + d.game_id + '"]');
      ordersAdjustCounts($sec, d.command_number, !!d.reserve);
      ordersDecrementRequestCount($sec);
      $sec.find('.orders-select-all-orders[data-game-id="' + d.game_id + '"]').prop('checked', false);
    });
  }).fail(function(xhr) {
...
$(document).on('click', '.orders-bulk-games-btn', function() {
  var $admin = $('.orders-admin');
  var url = $admin.data('games-bulk-url');
  if (!url) return;
  var ids = $('.orders-select-game:checked').map(function() { return parseInt($(this).val(), 10); }).get().filter(function(x) { return !isNaN(x); });
  if (!ids.length) {
    alert('Отметьте одну или несколько игр.');
    return;
  }
  if (!confirm('Данное действие необратимо. Удалить выбранные игры и все заявки по ним?')) return;
  adminPostJson(url, { ids: ids }).done(function(resp) {
    if (!resp || !resp.ok) return;
    (resp.deleted_game_ids || []).forEach(function(gid) {
      $('.orders-game-section[data-game-id="' + gid + '"]').remove();
    });
  }).fail(function(xhr) {
    var msg = (xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : xhr.statusText;
    alert('Ошибка удаления: ' + msg);
...
$(document).on('click', '.orders-order-delete', function(e) {
  e.preventDefault();
  if (!confirm('Данное действие необратимо. Вы уверены?')) return;
  var id = parseInt($(this).data('order-id'), 10);
  var $admin = $('.orders-admin');
  var url = $admin.data('orders-bulk-url');
  if (!url || isNaN(id)) return;
  adminPostJson(url, { ids: [id] }).done(function(resp) {
    if (!resp || !resp.ok || !resp.deleted || !resp.deleted.length) return;
...
$(document).on('click', '.orders-admin .games-delete-ajax', function(e) {
  e.preventDefault();
  if (!confirm('Данное действие необратимо. Удалить игру и все заявки по ней?')) return;
  var gid = parseInt($(this).data('game-id'), 10);
  var $admin = $('.orders-admin');
  var url = $admin.data('games-bulk-url');
  if (!url || isNaN(gid)) return;
  adminPostJson(url, { ids: [gid] }).done(function(resp) {
    if (!resp || !resp.ok) return;
    $('.orders-game-section[data-game-id="' + gid + '"]').remove();
  }).fail(function(xhr) {
    var msg = (xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : xhr.statusText;
...
$(document).on('change', '.games-select-all', function() {
  var checked = $(this).prop('checked');
  var $bulk = $(this).closest('.games-admin-bulk');
  if ($bulk.length) {
    $bulk.find('.games-select-game').prop('checked', checked);
    return;
  }
  $(this).closest('ul').find('.games-select-game').prop('checked', checked);
});

$(document).on('click', '.games-bulk-delete-btn', function() {
  var $wrap = $(this).closest('.games-admin-bulk');
  var url = $wrap.data('games-bulk-url');
  if (!url) return;
  var ids = $wrap.find('.games-select-game:checked').map(function() { return parseInt($(this).val(), 10); }).get().filter(function(x) { return !isNaN(x); });
  if (!ids.length) {
    alert('Отметьте одну или несколько игр.');
    return;
  }
  if (!confirm('Данное действие необратимо. Удалить выбранные игры?')) return;
  adminPostJson(url, { ids: ids }).done(function(resp) {
    if (!resp || !resp.ok) return;
    (resp.deleted_game_ids || []).forEach(function(gid) {
      $wrap.find('.orders-game-section[data-game-id="' + gid + '"]').remove();
    });
    $wrap.find('.games-select-all').prop('checked', false);
  }).fail(function(xhr) {
    var msg = (xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : xhr.statusText;
    alert('Ошибка удаления: ' + msg);
  });
...
$(document).on('click', '.games-admin-bulk .games-delete-ajax', function(e) {
  if ($(this).closest('.orders-admin').length) return;
  e.preventDefault();
  if (!confirm('Данное действие необратимо. Вы уверены?')) return;
  var gid = parseInt($(this).data('game-id'), 10);
  var $wrap = $(this).closest('.games-admin-bulk');
...
function gameAdminUpdateBadge($section, badge) {
  var $b = $section.find('.js-game-status-badge').first();
  if (!$b.length || !badge) return;
  $b.removeClass('orders-game-head__status--ok orders-game-head__status--warn orders-game-head__status--sold');
  if (badge.kind === 'ok') {
    $b.addClass('orders-game-head__status--ok');
  } else if (badge.kind === 'warn') {
    $b.addClass('orders-game-head__status--warn');
  } else {
    $b.addClass('orders-game-head__status--sold');
  }
  $b.text(badge.label);
}

$(document).on('click', '.game-flag-btn', function(e) {
  e.preventDefault();
  var $btn = $(this);
  var $sec = $btn.closest('[data-flags-url]');
  var url = $sec.data('flags-url');
  if (!url) return;
  var toggle = $btn.data('toggle');
  adminPostJson(url, { toggle: toggle }).done(function(resp) {
    if (!resp || !resp.ok) return;
    var $wrap = $sec.find('.orders-game-quick-flags');
    $wrap.find('[data-toggle="mark_few_seats"]').toggleClass('game-flag-btn--on', !!resp.mark_few_seats);
    $wrap.find('[data-toggle="reserve_enabled"]').toggleClass('game-flag-btn--on', !!resp.reserve_enabled);
    $wrap.find('[data-toggle="manual_sold_out"]').toggleClass('game-flag-btn--on', !!resp.manual_sold_out);
    if (resp.badge) {
      gameAdminUpdateBadge($sec, resp.badge);
    }
  }).fail(function(xhr) {
    var msg = (xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : xhr.statusText;
    alert('Ошибка: ' + msg);
...
function gamesAdminReorderDom($page, $sec, direction) {
  var d = $sec.attr('data-game-date') || '';
  var $same = $page.find('.orders-game-section').filter(function() {
    return (($(this).attr('data-game-date') || '') === d);
  });
  var idx = $same.index($sec);
  if (idx < 0) return;
  if (direction === 'up' && idx > 0) {
    $sec.insertBefore($same.eq(idx - 1));
  } else if (direction === 'down' && idx < $same.length - 1) {
    $sec.insertAfter($same.eq(idx + 1));
  }
}

$(document).on('click', '.games-admin-page .game-sort-btn', function(e) {
  e.preventDefault();
  var $btn = $(this);
  var $page = $btn.closest('.games-admin-page');
  var reorderUrl = $page.data('game-reorder-url');
  if (!reorderUrl) return;
  var $sec = $btn.closest('[data-game-id]');
  var gid = parseInt($sec.data('game-id'), 10);
  var dir = $btn.data('direction');
  if (isNaN(gid)) return;
  adminPostJson(reorderUrl, { game_id: gid, direction: dir }).done(function(resp) {
    if (resp && resp.ok && resp.changed) {
      gamesAdminReorderDom($page, $sec, dir);
    }
  }).fail(function(xhr) {
    var msg = (xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : xhr.statusText;
    alert('Ошибка сортировки: ' + msg);
  });
});
// end Заявки / игры
...
$(document).on('click','.ck-add-new-item',function(e){

  e.preventDefault()
  var parent = $(this).parent('.taxtarea-find') 
  var textareas = parent.find('textarea');
  textareas.each(function () {
    var textareaId = $(this).attr('id'); // Получаем id текущего поля textarea
    CKEDITOR.replace(textareaId); // Инициализируем CKEditor на данном поле
  });
...
$(document).on('click','.add-popup__layout',function(e){
  e.preventDefault();
  $('.add-popup').hide()
});
...
$(document).ready(function(){ 
  ...
  var block_st = $("#id_block_class").val();


  if(block_st=='slider') {

    $('.hidden-inputs__input').remove()

    var slider = $('.hidden-inputs__slider').clone().addClass('hidden-inputs__input')

    $('#id_block_class').after(slider)

  } else if (block_st=='photo') {

    $('.hidden-inputs__input').remove()

    var photo = $('.hidden-inputs__photo').clone().addClass('hidden-inputs__input')

    $('#id_block_class').after(photo)


  } else if (block_st=='video') {
    $('.hidden-inputs__input').remove()


    var video = $('.hidden-inputs__video').clone().addClass('hidden-inputs__input')
    $('#id_block_class').after(video)


  }
  

  $(document).on('click','#id_block_class',function(){

    var block_type = $("#id_block_class").val();
