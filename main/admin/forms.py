...
from admin.widgets_ckeditor import CKEditorWidgetCDN
...
# Страницы: CKEditorWidgetCDN — init из core/ckeditor, ckeditor.js с CDN (без collectstatic)
class PageForm(forms.ModelForm):
    text = forms.CharField(label='Текст страницы', required=False, widget=CKEditorWidgetCDN())
    class Meta:
        model = Page
        fields = "__all__"
    ...
    class Meta:
        ...
        widgets = {
            ...
            'telegram_bot_token': forms.TextInput(attrs={
                'class': 'input',
                'autocomplete': 'off',
                'placeholder': 'Токен Bot API',
            }),
            'telegram_default_group': forms.TextInput(attrs={
                'class': 'input',
                'placeholder': 'Например -1002487695898',
            }),
            'max_access_token': forms.TextInput(attrs={
                'class': 'input',
                'autocomplete': 'off',
                'placeholder': 'Токен MAX',
            }),
            'max_chat_id': forms.NumberInput(attrs={
                'class': 'input',
        ...
        labels = {
            ...
            'telegram_default_group': 'Telegram: ID чата по умолчанию',
            'max_access_token': 'MAX: токен API',
            'max_chat_id': 'MAX: ID чата по умолчанию',
