...
def _admin_parse_id_list(request):
    ids = []
...
def orders_bulk_delete(request):
    id_list = _admin_parse_id_list(request)
    if not id_list:
        return JsonResponse({'ok': False, 'error': 'no_ids'}, status=400)
    deleted = []
    for oid in id_list:
        try:
            order = GameOrder.objects.get(pk=oid)
...
@user_passes_test(lambda u: u.is_superuser)
@require_POST
def games_bulk_delete(request):
    id_list = _admin_parse_id_list(request)
    if not id_list:
        return JsonResponse({'ok': False, 'error': 'no_ids'}, status=400)
    qs = Games.objects.filter(pk__in=id_list)
    found = list(qs.values_list('id', flat=True))
    qs.delete()
    return JsonResponse({'ok': True, 'deleted_game_ids': found})


_GAME_FLAG_TOGGLES = ('mark_few_seats', 'reserve_enabled', 'manual_sold_out')


@user_passes_test(lambda u: u.is_superuser)
@require_POST
def game_admin_flags(request, pk):
    try:
    ...
    if toggle in _GAME_FLAG_TOGGLES:
        cur = getattr(game, toggle)
        setattr(game, toggle, not cur)
        game.save(update_fields=[toggle])
    else:
        updated = []
        for key in _GAME_FLAG_TOGGLES:
            if key in data:
                setattr(game, key, bool(data[key]))
                updated.append(key)
        if updated:
            game.save(update_fields=updated)
        else:
            return JsonResponse({'ok': False, 'error': 'no_field'}, status=400)
    game.refresh_from_db()
    badge = game.admin_capacity_badge()
    return JsonResponse({
        'ok': True,
        'mark_few_seats': game.mark_few_seats,
        'reserve_enabled': game.reserve_enabled,
        'manual_sold_out': game.manual_sold_out,
        'badge': badge,
    })
...
def game_reorder(request):
    ...
    try:
        ...
        return JsonResponse({'ok': False, 'error': 'not_found'}, status=404)
    if not game.date_date:
        return JsonResponse({'ok': False, 'error': 'no_date'}, status=400)
    with transaction.atomic():
        s = list(
            Games.objects.select_for_update()
            .filter(date_date=game.date_date)
            .order_by('-display_priority', 'id')
        )
        idx = next((i for i, g in enumerate(s) if g.id == game_id), None)
        if idx is None:
            return JsonResponse({'ok': False, 'error': 'not_in_list'}, status=400)
        ...
        for i, g in enumerate(s):
            g.display_priority = n - 1 - i
        Games.objects.bulk_update(s, ['display_priority'])
