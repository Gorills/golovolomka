...
class City(models.Model):
    ...
    max_chat_id = models.BigIntegerField(
        blank=True,
        null=True,
        verbose_name='MAX: ID чата',
        help_text='Если задан — уведомления о заявках по этому городу идут в этот чат MAX',
    )
...
class Games(models.Model):
    ...
    display_priority = models.IntegerField(
        default=0,
        verbose_name='Приоритет показа (в один день: больше — выше в списке)',
    )
    mark_few_seats = models.BooleanField(default=False, verbose_name='Показывать «мало мест»')
    reserve_enabled = models.BooleanField(default=True, verbose_name='Резерв включён')
    manual_sold_out = models.BooleanField(default=False, verbose_name='Вручную: мест нет')
    ...
    def main_people_count(self):
        """Сумма людей в заявках на основные места (не резерв)."""
        n = self.orders.filter(reserve=False).aggregate(Sum('command_number'))['command_number__sum']
        return n or 0

    def reserve_people_count(self):
        """Сумма людей в заявках в резерве."""
        n = self.orders.filter(reserve=True).aggregate(Sum('command_number'))['command_number__sum']
        return n or 0

    def reverve(self):
        """Есть свободные места в основном составе (до лимита number_of_seats)."""
        return self.main_people_count() < self.number_of_seats

    def closed(self):
        """Резерв исчерпан: занято не меньше лимита numbers_of_reserves."""
        return self.reserve_people_count() >= self.numbers_of_reserves
    ...
    def reserve_count(self):
        return self.reserve_people_count()

    def site_can_register(self):
        if self.manual_sold_out:
            return False
        if self.closed():
            return False
        if self.reverve():
            return True
        return bool(self.reserve_enabled)

    def admin_capacity_badge(self):
        if self.manual_sold_out:
            return {'label': 'Мест нет', 'kind': 'sold'}
        if self.reverve():
            return {'label': 'Места есть', 'kind': 'ok'}
        if not self.reserve_enabled:
            return {'label': 'Мест нет', 'kind': 'sold'}
        if self.closed():
            return {'label': 'Мест нет', 'kind': 'sold'}
        return {'label': 'Резерв', 'kind': 'warn'}
