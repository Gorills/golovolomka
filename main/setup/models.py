...
class BaseSettings(SingletonModel):
    ...
    debugging_mode = models.BooleanField(
        default=True,
        verbose_name='Режим разработки',
        help_text='Соответствует Django DEBUG на время запроса: подробная страница ошибок и отладочный контекст в шаблонах.',
    )
    ...
    telegram_bot_token = models.CharField(
        max_length=256,
        blank=True,
        verbose_name='Telegram: токен бота (Bot API)',
        help_text='Токен для отправки уведомлений о заявках',
    )
    telegram_default_group = models.CharField(
        max_length=250,
        blank=True,
        null=True,
        verbose_name='Telegram: ID чата/группы по умолчанию',
        help_text='Если у города не задана своя группа',
    )
    max_access_token = models.CharField(
        max_length=512,
        blank=True,
        verbose_name='MAX: токен API (Authorization)',
        help_text='Для Platform API MAX; без префикса Bearer, если не требуется документацией',
    )
    max_chat_id = models.BigIntegerField(
        blank=True,
        null=True,
        verbose_name='MAX: ID чата по умолчанию',
    )
